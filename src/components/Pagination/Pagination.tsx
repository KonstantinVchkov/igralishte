...
const Pagination = ({ itemsPerPage, totalItems, paginate, currentPage }:IRenderPagination) => {
  const pageNumbers = [];
  ...
  const handleNextOrPrevios = (e: React.MouseEvent<HTMLSpanElement, MouseEvent>, type: string) => {
    e.preventDefault();
  ...
  return (
    <nav>
      ...
      <span onClick={(e) => handleNextOrPrevios(e, "handlePrevios")}>{"<"}</span>
      {pageNumbers.map(number => (
        <li key={number} className={style.pageItem}>
          <a href={`http://localhost:3000/products?page=${number}`}
